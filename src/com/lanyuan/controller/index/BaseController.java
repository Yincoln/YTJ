...
/**
 * Controller基类，提供常用的方法，对于单表的增，删，改，查不需要再重复写。 如果对于单表的操作，有其他业务，则需要自己写方法处理
 * 
 * @author lanyuan
...
public class BaseController {
	...
	/**
	 * 1：默认返回的界面是 当前调用的Controller中RequestMapping注解地址路径 + list.jsp。
	 * 如：@RequestMapping("/user")。 地址是:/user/list.shtml 则返回的界面地址是 /user/list.jsp
	 * 2：返回自定义界面 必须转递一个参数 xxx.shtml?page_path=/user/list。则返回的界面地址是
	 * /user/list.jsp 3：接收的所有参数都会回传给界面。爱怎么获取就怎么获取！
	 * 
	 * @author lanyuan
	 * @Email：mmm333zzz520@163.com
	 * @date：2016-03-11
	...
	/**
	 * 1：默认返回的界面是 当前调用的Controller中RequestMapping注解地址路径 + add.jsp。
	 * 如：@RequestMapping("/user")。 地址是:/user/add.shtnl 则返回的界面地址是 /user/add.jsp
	 * 2：返回自定义界面 必须转递一个参数 xxx.shtml?page_path=/user/add。则返回的界面地址是 /user/add.jsp
	...
	/**
	 * 获取返回某一页面的按扭组,传入某url资源的ID, resId=XXX <br/>
	 * <b>author：</b><br/>
	...
	public List<FormMap> findByRes(HttpServletRequest request) {
		...
		List<FormMap> fs = baseService.findByNames(f);
		return fs;
	}

	/**
	 * 返回列表 1：如果返回单表的分页数据 必须传入 FormMap=XXX, XXX表示某表的实体类的名称， 如FormMap=UserFormMap
	 * (名称大小写必须一致)
	 * 
	 * 2：如果自定义复杂的Sql分页数据 必须传入 mapper_id=xxx.xxxx, xxx.xxxx表示某个xxx-mapper.xml 的
	 * namespace和sqlId <br/>
	 * 如 user-mapper.xml中。 namespace="userMapper" , 其中的你查询sql是对应的 <select
	 * id="findUserPage"...>....</select> 其中 id = findUserPage 这个就是sqlid <br/>
	...
	public PageView findByPage() throws Exception {
		...
		formMap.put("paging", pageView);
		pageView.setRecords(baseService.findByPage(formMap));
		return pageView;
	}

	/**
	 * 返回表的所有数据，json格式 1：如果返回单表的所有数据 必须传入 FormMap=XXX, XXX表示某表的实体类的名称，
	 * 如FormMap=UserFormMap (名称大小写必须一致)
	 * 
	 * 2：如果自定义复杂的Sql所有数据 必须传入 mapper_id=xxx.xxxx, xxx.xxxx表示某个xxx-mapper.xml 的
	 * namespace和sqlId <br/>
	 * 如 user-mapper.xml中。 namespace="userMapper" , 其中的你查询sql是对应的 <select
	 * id="findUserAll"...>....</select> 其中 id = findUserAll 这个就是sqlid <br/>
	...
	public List<FormMap> jsonlist() throws Exception {
		FormMap formMap = findHasHMap(FormMap.class);
		return baseService.findByNames(formMap);
	}

	/**
	 * 
	 * 1：根据ID删除数据 1：根据ID返回单表的数据 必须传入 FormMap=XXX, XXX表示某表的实体类的名称，
	 * 如FormMap=UserFormMap (名称大小写必须一致) 2：如果自定义复杂delete的sql语句 必须传入
	 * mapper_id=xxx.xxxx, xxx.xxxx表示某个xxx-mapper.xml 的 namespace和sqlId <br/>
	 * 如 user-mapper.xml中。 namespace="userMapper" , 其中的你查询sql是对应的 <delete
	 * id="deleteByIds"...>....</delete> 其中 id = deleteByIds 这个就是sqlid <br/>
	...
	public String deleteByIds() {
		FormMap formMap = findHasHMap(FormMap.class);
		try {
			baseService.deleteByIds(formMap);
			return "success";
		} catch (Exception e) {
	...
	/**
	 * 
	 * 1：默认返回的界面是 当前调用的Controller中RequestMapping注解地址路径 + edit.jsp。
	 * 如：@RequestMapping("/user")。 地址是:/user/edit.shtnl 则返回的界面地址是 /user/edit.jsp
	 * 2：返回自定义界面 必须转递一个参数 xxx.shtml?page_path=/user/edit。则返回的界面地址是
	 * /user/edit.jsp 3：接收的所有参数都会回传给界面。爱怎么获取就怎么获取！
	 * 
	 * 1：根据ID返回单表的数据 必须传入 FormMap=XXX, XXX表示某表的实体类的名称， 如FormMap=UserFormMap
	 * (名称大小写必须一致) 2：如果自定义复杂的Sql数据 必须传入 mapper_id=xxx.xxxx,
	 * xxx.xxxx表示某个xxx-mapper.xml 的 namespace和sqlId <br/>
	 * 如 user-mapper.xml中。 namespace="userMapper" , 其中的你查询sql是对应的 <select
	 * id="findById"...>....</select> 其中 id = findById 这个就是sqlid <br/>
	...
	public String edit(Model model, String id) throws Exception {
		FormMap formMap = findHasHMap(FormMap.class);
		model.addAllAttributes(formMap);
		FormMap map = baseService.findById(formMap);
	...
	/**
	 * 
	 * 1：根据ID返回json数据 1：根据ID返回单表的数据 必须传入 FormMap=XXX, XXX表示某表的实体类的名称，
	 * 如FormMap=UserFormMap (名称大小写必须一致) 2：如果自定义复杂的Sql数据 必须传入 mapper_id=xxx.xxxx,
	 * xxx.xxxx表示某个xxx-mapper.xml 的 namespace和sqlId <br/>
	 * 如 user-mapper.xml中。 namespace="userMapper" , 其中的你查询sql是对应的 <select
	 * id="findById"...>....</select> 其中 id = findById 这个就是sqlid <br/>
	 * 那么前端参数必须是 mapper_id=userMapper.findById
	...
	public Map findByIdJson(Model model, String id) throws Exception {
		FormMap formMap = findHasHMap(FormMap.class);
		model.addAllAttributes(formMap);
		return baseService.findById(formMap);
	}

	/**
	 * 
	 * 1：根据某字段判断表是否存在数据 1：查询单表的数据 必须传入 FormMap=XXX, XXX表示某表的实体类的名称，
	 * 如FormMap=UserFormMap (名称大小写必须一致) 2：如果自定义的Sql查询数据 必须传入 mapper_id=xxx.xxxx,
	 * xxx.xxxx表示某个xxx-mapper.xml 的 namespace和sqlId <br/>
	 * 如 user-mapper.xml中。 namespace="userMapper" , 其中的你查询sql是对应的 <select
	 * id="isExist"...>....</select> 其中 id = isExist 这个就是sqlid <br/>
	...
	public boolean isExist() {
		FormMap formMap = findHasHMap(FormMap.class);
		List<FormMap> fs = baseService.findByNames(formMap);
	...
	/**
	 * 
	 * 1：修改单表的数据 必须传入 FormMap=XXX, XXX表示某表的实体类的名称， 如FormMap=UserFormMap
	 * (名称大小写必须一致) 2：如果自定义的update Sql 必须传入 mapper_id=xxx.xxxx,
	 * xxx.xxxx表示某个xxx-mapper.xml 的 namespace和sqlId <br/>
	 * 如 user-mapper.xml中。 namespace="userMapper" , 其中的你查询sql是对应的 <select
	 * id="editEntity"...>....</select> 其中 id = editEntity 这个就是sqlid <br/>
	...
	public String update() throws Exception {
		try {
			FormMap formMap = findHasHMap(FormMap.class);
			baseService.editEntity(formMap);
			return "success";
		} catch (Exception e) {
	...
	/**
	 * 
	 * 1：插入单表的数据 必须传入 FormMap=XXX, XXX表示某表的实体类的名称， 如FormMap=UserFormMap
	 * (名称大小写必须一致) 2：如果自定义的insert Sql 必须传入 mapper_id=xxx.xxxx,
	 * xxx.xxxx表示某个xxx-mapper.xml 的 namespace和sqlId <br/>
	 * 如 user-mapper.xml中。 namespace="addEntity" , 其中的你查询sql是对应的 <select
	 * id="addEntity"...>....</select> 其中 id = addEntity 这个就是sqlid <br/>
	 ...
	 * @author lanyuan Email：mmm333zzz520@163.com date：2014-5-25
	 * @param request
	 */
	/*
	...
	public String[] findUrl() {
		RequestMapping annotation = this.getClass().getAnnotation(
