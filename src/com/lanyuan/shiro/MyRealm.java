...
import com.lanyuan.service.ResService;
...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
		...
		if (loginName != null) {
			...
			List<ResFormMap> rs = resService.findByNames(resFormMap);
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		List<UserFormMap> userFormMaps = resService.findByNames(userFormMap);
		if (userFormMaps.size() != 0) {
			...
			session.setAttribute("userSessionAccountName",userFormMaps.get(0).get("accountName"));
			//session.setAttribute("userSessionRoleKey", userFormMaps.get(0).get("roleKey"));
			//userId对应的playerId
			PlayerFormMap playerFormMap = new PlayerFormMap();
			playerFormMap.put("userId", userFormMaps.get(0).get("id"));
			//根据userId查找对应的playerId
			playerFormMap.put("mapper_id", "PlayerMapper.findPlayerByUserId");
			try {
				playerFormMap = resService.findById(playerFormMap);
			} catch (Exception e) {
				e.printStackTrace();
			}
			int playerId = Integer.parseInt(playerFormMap.getStr("player_id"));
			session.setAttribute("playerSessionId", playerId);
